    ...
    try {
      ...
      if (data.success) {
        if (data.authenticated) {
          ...
          setInfo(data.message || "Please verify your email, then log in.")
          router.push("/login")
        }
      } else {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      const response = await fetch("/api/auth/google", {
        ...
        body: JSON.stringify({ role: formData.role || "student" }),
      })
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <h1 className="text-2xl font-bold">EduTech VR</h1>
          </div>
          <CardTitle className="text-2xl">Create Account</CardTitle>
          <CardDescription>Join thousands of learners in the future of education</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {error && <div className="p-3 text-sm text-red-600 bg-red-50 border border-red-200 rounded-md">{error}</div>}
          {info && <div className="p-3 text-sm text-blue-700 bg-blue-50 border border-blue-200 rounded-md">{info}</div>}
